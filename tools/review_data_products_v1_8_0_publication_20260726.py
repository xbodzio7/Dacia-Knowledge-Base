...
def verify_state() -> None:
    ...
    ensure(state.get("phase") == "Data Products v1.8.0 Publication", "project phase differs")
    ensure(
        state.get("current_package", {}).get("name") == "Data Products v1.8.0 Publication",
        "current package differs",
    )
    ensure(state.get("current_package", {}).get("status") == "complete", "current package is not complete")
    ensure(
        state.get("next_package", {}).get("name") == "Cross-Model Navigation Usability Review",
        "state next package differs",
    )
    baseline = state.get("baseline", {})
    ensure(baseline.get("tests") == 1022, "test baseline differs")
