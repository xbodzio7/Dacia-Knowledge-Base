...
assert state['next_package']['package_id'] == 'data_products_v1_15_0_publication_001'
...
if gh release view data-products-v1.15.0 >/dev/null 2>&1; then
  echo 'Release already exists; refusing to rewrite it.' >&2
  exit 1
fi
if git show-ref --tags --verify --quiet refs/tags/data-products-v1.15.0; then
...
expected = {
    ...
    'source_coverage_matrix_html': (
        'contents/source-coverage/portfolio_source_coverage_matrix.html'
    ),
}
...
source_matrix = (
    workspace / expected['source_coverage_matrix_html']
).read_text(encoding='utf-8')
assert source_matrix.startswith('<!doctype html>')
assert source_matrix.count('<tr>') == 34
assert 'Portfolio Source Coverage Matrix' in source_matrix
assert 'href="http' not in source_matrix.lower()
assert '<script' not in source_matrix.lower()
PY
...
unzip -p \
  "${RUNNER_TEMP}/release-a/dacia-knowledge-base-data-products-v1.15.0.zip" \
  RELEASE_NOTES.md > "${RUNNER_TEMP}/release-notes.md"
if [[ "$(grep -c '^## v1.15.0 portfolio source coverage matrix$' "${RUNNER_TEMP}/release-notes.md")" != "1" ]]; then
  echo 'Expected exactly one v1.15.0 release-notes section.' >&2
  exit 1
fi
grep -F 'source_coverage_matrix_html' "${RUNNER_TEMP}/release-notes.md"
grep -F 'Public `data-products-v1.14.1` remains immutable' "${RUNNER_TEMP}/release-notes.md"

gh release create data-products-v1.15.0 \
  "${RUNNER_TEMP}/release-a/dacia-knowledge-base-data-products-v1.15.0.zip" \
  "${RUNNER_TEMP}/release-a/data-product-release-manifest.json" \
  "${RUNNER_TEMP}/release-a/SHA256SUMS" \
  --target "${PUBLICATION_SOURCE_SHA}" \
  --title 'Dacia Knowledge Base Data Products v1.15.0' \
  --notes-file "${RUNNER_TEMP}/release-notes.md"
RELEASE_ID="$(gh release view data-products-v1.15.0 --json databaseId -q '.databaseId')"

mkdir "${RUNNER_TEMP}/public"
gh release download data-products-v1.15.0 --dir "${RUNNER_TEMP}/public"
diff -q \
  "${RUNNER_TEMP}/release-a/dacia-knowledge-base-data-products-v1.15.0.zip" \
  "${RUNNER_TEMP}/public/dacia-knowledge-base-data-products-v1.15.0.zip"
diff -q \
  "${RUNNER_TEMP}/release-a/data-product-release-manifest.json" \
...
python tools/record_data_products_v1_15_0_publication_20260804.py
python tools/dkb.py project-state --apply
python tools/dkb.py documentation-baseline --apply
python tools/dkb.py project-state --check

rm -f tools/publish_data_products_v1_15_0_20260804.sh
rm -f tools/record_data_products_v1_15_0_publication_20260804.py

git config user.name 'github-actions[bot]'
git config user.email '41898282+github-actions[bot]@users.noreply.github.com'
git add -A
git commit -m 'release(data-products): record v1.15.0 publication'
