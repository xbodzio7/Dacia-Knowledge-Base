#!/usr/bin/env python3
"""Close reporting dependencies resolved by the Stepway Extreme source-gap import."""
...
TARGET_CONFIGURATION = "sandero_stepway_iii_extreme_ecog120_manual"
TARGET_SOURCE = "src_pl_sandero_stepway_extreme_ecog120_mt_20260626"
TARGETS = {
...
def targeted_key(key: object) -> bool:
    ...
    return (
        ...
        and (domain, attribute, "" if fuel == "none" else fuel) in TARGETS
    )
...
def filtered_evidence(
    ...
    output = dict(payload)
    output["decisions"] = [
        item for item in decisions if not targeted_decision(item)
    ]
    return output, removed


def filtered_source_review(
    payload: dict[str, object],
) -> tuple[dict[str, object], int]:
    ...
    output = dict(payload)
    output["review_triage_keys"] = [
        item for item in keys if not targeted_key(item)
    ]
    return output, removed
...
def reconciliation_outputs() -> tuple[str, str]:
    payload, markdown = reconciliation.build_from_paths(
        ROOT,
        reconciliation.DEFAULT_LEDGER,
        reconciliation.DEFAULT_REVIEW,
    )
    ...
    for path in EVIDENCE_PATHS:
        ...
        if removed not in {0, 6}:
            raise DependencyError(
                f"expected zero or six resolved evidence decisions in {path}, "
                f"found {removed}"
            )
        path.write_text(canonical(evidence), encoding="utf-8")
    source_review, review_removed = filtered_source_review(
        load_object(SOURCE_REVIEW)
    )
...
def check() -> None:
    for path in EVIDENCE_PATHS:
        ...
        if remaining:
            raise DependencyError(
                f"{remaining} resolved evidence decisions remain in {path}"
            )
    _, review_remaining = filtered_source_review(load_object(SOURCE_REVIEW))
    if review_remaining:
        raise DependencyError(
            f"{review_remaining} resolved source-review keys remain"
        )
    json_text, markdown = reconciliation_outputs()
    if (
        (ROOT / reconciliation.DEFAULT_JSON).read_text(encoding="utf-8")
        != json_text
    ):
        raise DependencyError("coverage reconciliation JSON is stale")
    if (
        (ROOT / reconciliation.DEFAULT_MARKDOWN).read_text(encoding="utf-8")
        != markdown
    ):
        raise DependencyError("coverage reconciliation Markdown is stale")
    print("Stepway Extreme reporting dependencies: PASS")
