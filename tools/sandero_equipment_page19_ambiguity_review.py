#!/usr/bin/env python3
"""Build or verify the authored Sandero brochure page-19 equipment ambiguity review."""
...
REVIEW_VERSION = 1
REVIEW_KIND = "sandero_equipment_page19_ambiguity_review"
REVIEWED_ON = "2026-07-28"
DEFAULT_PRIORITIZATION = Path("data/reporting/verified_pdf_candidate_residual_gap_prioritization.json")
DEFAULT_JSON = Path("data/reporting/sandero_equipment_page19_ambiguity_review.json")
DEFAULT_MARKDOWN = Path("data/reporting/sandero_equipment_page19_ambiguity_review.md")
PACKAGE_ID = "residual_gap_014"
...
NEXT_PACKAGE = "Sandero Stepway Equipment Page 18 Ambiguity Review"
DECISION_STATUSES = {"covered"}
TRIMS = ("essential", "expression", "journey")


class SanderoEquipmentPage19ReviewError(RuntimeError):
    pass
...
def ensure(condition: bool, message: str) -> None:
    if not condition:
        raise SanderoEquipmentPage19ReviewError(message)
...
def load_json_object(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise SanderoEquipmentPage19ReviewError(f"cannot read {label}: {exc}") from exc
    except json.JSONDecodeError as exc:
        raise SanderoEquipmentPage19ReviewError(f"invalid JSON in {label}: {exc}") from exc
...
def sha256(path: Path) -> str:
    ...
    try:
        ...
        raise SanderoEquipmentPage19ReviewError(f"cannot read archived source: {exc}") from exc
...
DECISIONS = (
    {
        "candidate_id": "95fc01e2757ac30f1c2f3a5f43cc0b52f65e6097240d2179401eac2b2a17b32c",
        "line_start": 20,
        "line_end": 20,
        "exact_text": "Regulator i ogranicznik prędkości                                       •           •           •",
        "decision": "covered",
        "selected": [
            availability_signature("cruise_control", "standard"),
            availability_signature("speed_limiter", "standard"),
        ],
        "row_context": "complete combined speed-limiter and cruise-control row",
        "source_availability": {
            "essential": "standard",
            "expression": "standard",
            "journey": "standard",
        ...
        "rationale": (
            "The 2026-02-02 brochure prints one complete combined row for cruise control and the speed limiter, with both "
            "functions standard in Essential, Expression and Journey. The two attached signatures preserve these as distinct "
            "attributes, while the four exact Expression and Journey vehicle-sheet records confirm the standard state without "
            "substituting either function for the other. The candidate is covered as an authored review only and does not "
            "approve an import or project values between configurations."
        ),
    },
)
...
def read_source_row(repository: Path) -> dict[str, str]:
    ...
    try:
        ...
        raise SanderoEquipmentPage19ReviewError(f"cannot read sources.csv: {exc}") from exc
...
def validate_prioritization(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    ensure(len(matches) == 1, "residual_gap_014 package differs")
...
def verify_source(repository: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "page": SOURCE_PAGE,
        "review_basis": "authored visual review of the archived page-19 equipment matrix",
    }
...
def build_review(prioritization: Mapping[str, Any], repository: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "semantic_boundaries": {
            ...
            "combined_row_maps_to_two_distinct_standard_attributes": True,
            "essential_expression_and_journey_source_states_remain_standard": True,
            "no_configuration_projection_is_created": True,
        },
        "next_package": {
            "name": NEXT_PACKAGE,
            "status": "planned",
            "goal": "Review the 1 ambiguous equipment candidate from Sandero Stepway brochure page 18 against its 2 preserved evidence signatures without creating master-data rows or approved import specifications.",
        },
    }
...
def ensure_safe_output(repository: Path, path: Path) -> Path:
    ...
    for restricted in (repository / "data/master", repository / "data/imports"):
        try:
            ...
            continue
        raise SanderoEquipmentPage19ReviewError(f"output path is restricted: {path}")
    return resolved
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.verify:
            ...
            print("Sandero equipment page-19 ambiguity review: PASS")
        else:
        ...
        return 0
    except SanderoEquipmentPage19ReviewError as exc:
