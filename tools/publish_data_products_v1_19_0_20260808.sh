...
assert state['next_package']['package_id']=='data_products_v1_19_0_publication_001'
...
if gh release view data-products-v1.19.0 >/dev/null 2>&1; then echo "Release already exists." >&2; exit 1; fi
if git show-ref --tags --verify --quiet refs/tags/data-products-v1.19.0; then echo "Tag already exists." >&2; exit 1; fi
...
readiness=json.loads((root/'data/reporting/configurator_ui_commercial_choice_readiness_20260807.json').read_text(encoding='utf-8'))
coverage=readiness['coverage']
assert coverage['non_appearance_commercial_items']==34
assert coverage['selector_offer_rows']==167
assert coverage['selector_offer_rows_with_price']==162
assert coverage['selector_offer_rows_without_price']==5
assert coverage['model_families_covered']==6
assert readiness['readiness']['pairwise_dependency_conflict_graph']=='not_modeled_generically'
assert readiness['readiness']['verified_final_price_after_multiple_commercial_choices']=='not_ready_generically'
assert readiness['master_data_mutated'] is False
...
for dir in release-a release-b; do
  python tools/dkb.py data-product-release --output-directory "${RUNNER_TEMP}/${dir}" --version 1.19.0 --commit-sha "${PUBLICATION_SOURCE_SHA}"
done
diff -qr "${RUNNER_TEMP}/release-a" "${RUNNER_TEMP}/release-b"
python tools/dkb.py data-product-release --output-directory "${RUNNER_TEMP}/release-a" --version 1.19.0 --commit-sha "${PUBLICATION_SOURCE_SHA}" --verify
...
assert manifest['release_version']=='1.19.0'
...
archive=release/'dacia-knowledge-base-data-products-v1.19.0.zip'
...
unzip -p "${RUNNER_TEMP}/release-a/dacia-knowledge-base-data-products-v1.19.0.zip" RELEASE_NOTES.md > "${RUNNER_TEMP}/release-notes.md"
grep -F "v1.19.0" "${RUNNER_TEMP}/release-notes.md"

REMOTE_MAIN="$(git ls-remote origin refs/heads/main | awk '{print $1}')"
[[ "${REMOTE_MAIN}" == "${PUBLICATION_SOURCE_SHA}" ]] || { echo "main moved before publication" >&2; exit 1; }
if gh release view data-products-v1.19.0 >/dev/null 2>&1; then echo "Release appeared during verification." >&2; exit 1; fi

gh release create data-products-v1.19.0 \
  "${RUNNER_TEMP}/release-a/dacia-knowledge-base-data-products-v1.19.0.zip" \
  "${RUNNER_TEMP}/release-a/data-product-release-manifest.json" \
  "${RUNNER_TEMP}/release-a/SHA256SUMS" \
  --target "${PUBLICATION_SOURCE_SHA}" --title "Dacia Knowledge Base Data Products v1.19.0" --notes-file "${RUNNER_TEMP}/release-notes.md"

RELEASE_ID="$(gh release view data-products-v1.19.0 --json databaseId -q '.databaseId')"
mkdir "${RUNNER_TEMP}/public"
gh release download data-products-v1.19.0 --dir "${RUNNER_TEMP}/public"
for name in dacia-knowledge-base-data-products-v1.19.0.zip data-product-release-manifest.json SHA256SUMS; do diff -q "${RUNNER_TEMP}/release-a/${name}" "${RUNNER_TEMP}/public/${name}"; done
python tools/dkb.py data-product-release --output-directory "${RUNNER_TEMP}/public" --version 1.19.0 --commit-sha "${PUBLICATION_SOURCE_SHA}" --verify
...
python tools/record_data_products_v1_19_0_publication_20260808.py
python tools/dkb.py project-state --apply
python tools/dkb.py documentation-baseline --apply
python tools/dkb.py project-state --check
rm -f tools/publish_data_products_v1_19_0_20260808.sh tools/record_data_products_v1_19_0_publication_20260808.py

git config user.name "github-actions[bot]"
git config user.email "41898282+github-actions[bot]@users.noreply.github.com"
git add -A
git commit -m "release(data-products): record v1.19.0 publication"
