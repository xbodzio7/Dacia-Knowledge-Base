#!/usr/bin/env python3
"""Build or verify the authored Bigster brochure page-22 equipment ambiguity review."""
...
REVIEW_VERSION = 1
REVIEW_KIND = "bigster_equipment_page22_ambiguity_review"
REVIEWED_ON = "2026-07-28"
DEFAULT_PRIORITIZATION = Path("data/reporting/verified_pdf_candidate_residual_gap_prioritization.json")
DEFAULT_JSON = Path("data/reporting/bigster_equipment_page22_ambiguity_review.json")
DEFAULT_MARKDOWN = Path("data/reporting/bigster_equipment_page22_ambiguity_review.md")
PACKAGE_ID = "residual_gap_009"
...
NEXT_PACKAGE = "Jogger Equipment Page 20 Ambiguity Review"
DECISION_STATUSES = {"covered", "partially_covered"}
TRIMS = ("essential", "expression", "journey", "extreme")

class BigsterEquipmentPage22ReviewError(RuntimeError):
    pass
...
def ensure(condition: bool, message: str) -> None:
    if not condition:
        raise BigsterEquipmentPage22ReviewError(message)
...
def load_json_object(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise BigsterEquipmentPage22ReviewError(f"cannot read {label}: {exc}") from exc
    except json.JSONDecodeError as exc:
        raise BigsterEquipmentPage22ReviewError(f"invalid JSON in {label}: {exc}") from exc
...
def sha256(path: Path) -> str:
    ...
    try:
        ...
        raise BigsterEquipmentPage22ReviewError(f"cannot read archived source: {exc}") from exc
...
DECISIONS = ({'candidate_id': 'f8361f7eaf42bbf01040e78423f7190b9de86d9aff2ae02d12bae1666a3330ba',
  'line_start': 22,
  'line_end': 22,
  ...
  'rationale': 'This complete row preserves manual air conditioning as standard in Essential and unavailable in '
               'Expression, Journey and Extreme.'},
 {'candidate_id': '1aad4a730531585a37da9d3c73eb9115f9cca5a2eddd495d54e7a01e83f679a8',
  'line_start': 24,
  'line_end': 24,
  'exact_text': 'Klimatyzacja automatyczna dwustrefowa',
  ...
  'selected': [{'attribute_code': 'automatic_climate_control', 'availability_status': 'not_available'},
               ...
               {'attribute_code': 'dual_zone_climate_control', 'availability_status': 'not_available'},
               {'attribute_code': 'dual_zone_climate_control', 'availability_status': 'standard'},
               {'attribute_code': 'rear_air_vents', 'availability_status': 'not_available'},
               {'attribute_code': 'rear_air_vents', 'availability_status': 'standard'}],
  'row_context': 'first line of the dual-zone automatic-climate row with rear vents',
  'source_availability': {'essential': 'all three facts not_available',
                          'expression': 'all three facts standard',
                          'journey': 'all three facts standard',
                          'extreme': 'all three facts standard'},
  'rationale': 'This is the first line of one three-line visual row: automatic climate control, dual-zone operation '
               'and rear vents are unavailable in Essential and standard in the other trims. The three attributes '
               'remain distinct.'},
 {'candidate_id': '01f11f75e7390b17ccd40b4d46b23c55cb30ebfb71fdb6f292c07ae977b2f528',
  'line_start': 29,
  'line_end': 29,
  'exact_text': 'Lusterka składane elektrycznie                                      -            •             '
                '•             •',
  'decision': 'covered',
  'selected': [{'attribute_code': 'side_mirrors_folding', 'availability_status': 'not_available'},
               {'attribute_code': 'side_mirrors_folding', 'availability_status': 'standard'}],
  'row_context': 'complete electrically folding mirrors row',
  'source_availability': {'essential': 'not_available',
                          'expression': 'standard',
                          'journey': 'standard',
                          'extreme': 'standard'},
  'rationale': 'This complete row preserves electrically folding mirrors as unavailable in Essential and standard in '
               'all higher trims.'},
 {'candidate_id': 'ad2df36a0dce4c4d725438894b52fd307695f3bd0cab142a509be8aa67859473',
  'line_start': 40,
  'line_end': 40,
  ...
  'row_context': 'first line of the centre-console-with-armrest-and-storage row',
  'source_availability': {'essential': 'not_available',
                          'expression': 'standard',
                          'journey': 'not_available',
                          'extreme': 'not_available'},
  'rationale': 'This is the first line of the Expression centre-console row. Essential has a low console, while '
               'Journey and Extreme use the separate high-console row; the variants are not merged.'},
 {'candidate_id': '4119069370e836ed8bf21cc28b8e9c5a9d889d4abbbfe789c1c9c229fb1d2d34',
  'line_start': 48,
  'line_end': 48,
  ...
  'row_context': 'complete wireless-charging row',
  'source_availability': {'essential': 'not_available',
                          'expression': 'not_available',
                          'journey': 'standard',
                          'extreme': 'standard'},
  'rationale': 'This complete row preserves wireless charging as unavailable in Essential and Expression and standard '
               'in Journey and Extreme.'},
 {'candidate_id': 'e114af3d6918ae90c0ad19482028be640e718e50f989a1c282e0473f16759684',
  'line_start': 97,
  'line_end': 97,
  'exact_text': 'kierownica podgrzewana',
  'decision': 'partially_covered',
  ...
  'row_context': 'final line of the Winter package row',
  'source_availability': {'essential': 'not_available',
                          'expression': 'optional_package',
                          'journey': 'optional_package',
                          'extreme': 'optional_package'},
  'rationale': 'This fragment closes the Winter package row. The heated steering wheel is unavailable in Essential and '
               'optional or package-bound in the other trims; it is not promoted to standard.'},
 {'candidate_id': 'be9a3c17edaa4a791a8cbec07a987499acd8691f37c0712c06f5434d9f519437',
  'line_start': 100,
  'line_end': 100,
  'exact_text': 'podgrzewana przednia szyba',
  'decision': 'partially_covered',
  'selected': [{'attribute_code': 'heated_windscreen', 'availability_status': 'not_available'},
               {'attribute_code': 'heated_windscreen', 'availability_status': 'optional'}],
  'row_context': 'final line of the Winter Plus package row',
  'source_availability': {'essential': 'not_available',
                          'expression': 'optional_package',
                          'journey': 'optional_package',
                          'extreme': 'optional_package'},
  'rationale': 'This fragment closes the Winter Plus package row. The heated windscreen is unavailable in Essential '
               'and optional or package-bound in the other trims; it is not promoted to standard.'})
...
def read_source_row(repository: Path) -> dict[str, str]:
    ...
    try:
        ...
        raise BigsterEquipmentPage22ReviewError(f"cannot read sources.csv: {exc}") from exc
...
def validate_prioritization(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    ensure(len(matches) == 1, "residual_gap_009 package differs")
    ...
    ensure(package.get("candidate_count") == 7, "package candidate count differs")
    ensure(package.get("evidence_signature_count") == 18, "package evidence signature count differs")
    ensure(package.get("evidence_record_count") == 126, "package evidence record count differs")
    candidates = package.get("candidates")
    ensure(isinstance(candidates, list) and len(candidates) == 7, "package candidates differ")
    return package
...
def build_review(prioritization: Mapping[str, Any], repository: Path) -> dict[str, Any]:
    ...
    for authored in DECISIONS:
        ...
        for item in selected:
            ...
            for record in records:
                ...
                ensure(str(record.get("configuration_code", "")).startswith("bigster_"), "selected evidence model boundary differs")
    ...
    ensure(len(decision_ids) == len(set(decision_ids)) == 7, "authored candidate assignment differs")
    counts = Counter(item["authored_decision"] for item in decisions)
    ensure(counts == Counter({"partially_covered": 4, "covered": 3}), "authored decision distribution differs")
    ...
    ensure((selected_signature_count, selected_record_count) == (18, 126), "selected evidence totals differ")
    return {
        ...
        "scope": {"candidate_count": 7, "source_code": SOURCE_CODE, "model_code": "bigster", "domain": "equipment_matrix", "page": SOURCE_PAGE, "input_coverage_status": "ambiguous"},
        ...
        "summary": {"candidate_count": 7, "decision_counts": {"covered": 3, "partially_covered": 4},
                    "selected_evidence_signature_count": selected_signature_count, "selected_evidence_record_count": selected_record_count,
                    "rejected_attached_signature_count": 18-selected_signature_count, "rejected_attached_record_count": 126-selected_record_count,
                    "candidates_with_selected_evidence": 7, "candidates_without_selected_evidence": 0},
        "decisions": decisions,
        "semantic_boundaries": {"review_is_not_import_approval": True, "bullet_option_and_dash_symbols_remain_distinct": True,
                                 "multi_line_rows_are_reviewed_as_visual_units": True, "automatic_climate_dual_zone_and_rear_vents_remain_distinct": True,
                                 "centre_console_variants_remain_distinct": True, "winter_package_components_remain_optional_or_unavailable": True,
                                 "no_configuration_projection_is_created": True},
        "next_package": {"name": NEXT_PACKAGE, "status": "planned",
                         "goal": "Review the 5 ambiguous equipment candidates from Jogger brochure page 20 against their 10 preserved evidence signatures without creating master-data rows or approved import specifications."},
    }

def render_markdown(payload: Mapping[str, Any]) -> str:
    summary = payload["summary"]
    lines = ["# Bigster Equipment Page 22 Ambiguity Review", "",
    ...
    lines.extend(["", "## Safety boundary", "", "- no file under `data/master` is changed;", "- no approved import specification is created or changed;",
                  ...
                  "- Winter-package components do not inherit standard status from other trims or records;", "- climate functions and centre-console variants remain separate attributes inside their visual rows;",
...
def ensure_safe_output(repository: Path, path: Path) -> Path:
    ...
    for restricted in (repository / "data/master", repository / "data/imports"):
        try:
            ...
            continue
        raise BigsterEquipmentPage22ReviewError(f"output path is restricted: {path}")
    return resolved
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.verify:
            ...
            print("Bigster equipment page-22 ambiguity review: PASS")
        else:
            write_atomic(json_path, expected_json)
            write_atomic(markdown_path, markdown)
            print(f"JSON report written to {json_path}")
            print(f"Markdown report written to {markdown_path}")
        print("Candidates reviewed: 7")
        print("Selected evidence signatures: 18")
        print("Selected evidence records: 126")
        return 0
    except BigsterEquipmentPage22ReviewError as exc:
