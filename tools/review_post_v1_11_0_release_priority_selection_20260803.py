...
def historical_completeness_summary(payload: dict[str, Any]) -> dict[str, Any]:
    """Return the immutable completeness receipt recorded by this 2026-08-03 review.

    The current completeness product is intentionally allowed to evolve as later
    source-backed observations fill gaps or add configurations.  This dated review
    must therefore verify against the values that were actually recorded on
    2026-08-03 rather than recomputing that historical decision from today's state.
    """
    if not isinstance(payload.get("summary"), dict):
        raise RuntimeError("current completeness summary is missing")
    return {
        "active_configuration_count": 81,
        "completeness_scope_count": 23,
        "missing_technical_count": 81,
        "missing_equipment_count": 12,
        "candidate_count": 7,
        "exhausted_source_candidate_count": 7,
