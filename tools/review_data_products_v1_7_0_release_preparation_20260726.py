...
TARGET_AUDIT = ROOT / "project" / "releases" / "data-products-v1.7.0-publication-audit.json"
...
def verify_report(payload: Mapping[str, Any]) -> None:
    ...
    ensure(payload.get("kind") == "data_products_v1_7_0_release_preparation", "unexpected preparation kind")
    ...
    ensure(payload.get("source_selection_review") == "post_brochure_priority_selection_review.json", "source selection review differs")
    ...
    ensure(identity.get("publication_mode") == "manual_preflight_publish_audit", "publication mode differs")
    ensure(identity.get("final_source_commit") == "assigned_after_squash_merge", "final source commit must remain deferred to merge")
    ensure(identity.get("final_asset_identity") == "assigned_by_post_merge_preflight", "final asset identity must remain deferred to preflight")
    ...
    ensure(verification.get("previous_public_release") == "data-products-v1.6.1", "previous public release differs")
    ensure(verification.get("previous_public_release_commit") == "4b77571c788b862a6543161b9343a35f464bd7c6", "previous release commit differs")
    ...
    ensure(receipt.get("identity_status") == "diagnostic_only_not_publication_identity", "diagnostic receipt is presented as final")
    ensure(isinstance(receipt.get("workflow_run"), int), "workflow run is missing")
    ensure(SHA40.fullmatch(str(receipt.get("source_commit", ""))) is not None, "diagnostic source commit is invalid")
    ...
    for name, record in assets.items():
        ensure(isinstance(record, dict), f"diagnostic asset record is invalid: {name}")
        ensure(isinstance(record.get("size_bytes"), int) and record["size_bytes"] > 0, f"asset size is invalid: {name}")
        ensure(SHA256.fullmatch(str(record.get("sha256", ""))) is not None, f"asset hash is invalid: {name}")
    ensure(SHA256.fullmatch(str(receipt.get("workspace_index_sha256", ""))) is not None, "workspace index hash is invalid")
    ...
    ensure(isinstance(next_package, dict) and next_package.get("name") == "Data Products v1.7.0 Preflight", "next package differs")
...
def verify_repository(payload: Mapping[str, Any]) -> None:
    ...
    ensure(len(active_codes) == len(active), "active configuration code is duplicated")
    ensure(len(active) >= 72, "active configuration count regressed below v1.7.0")

    scopes = tuple(discover_scopes(ROOT))
    ensure(len(scopes) >= 19, "repository comparison scope count regressed below v1.7.0")
    ...
    ensure(set(scoped_codes) == active_codes, "current scope union differs from active configurations")
    ensure(
        sum(len(scope.configuration_codes) * (len(scope.configuration_codes) - 1) // 2 for scope in scopes) >= 114,
        "current scope pair count regressed below v1.7.0",
    )
    ...
    ensure(TARGET_RELEASE.is_file() == TARGET_AUDIT.is_file(), "v1.7.0 publication records are incomplete")
    if TARGET_AUDIT.is_file():
        ...
        ensure(audit.get("target_commit_sha") == "99e0e19b86cad6eae619f37702464e6a5a761cd8", "v1.7.0 target commit differs")
    ...
    ensure(baseline.get("selected_configuration_count") == 72, "frozen report configuration count differs")
    ensure(baseline.get("scope_group_count") == 19, "frozen report scope count differs")
