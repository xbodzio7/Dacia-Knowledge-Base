#!/usr/bin/env python3
"""Build or verify the authored Bigster brochure page-21 equipment ambiguity review."""
...
REVIEW_VERSION = 1
REVIEW_KIND = "bigster_equipment_page21_ambiguity_review"
REVIEWED_ON = "2026-07-28"
DEFAULT_PRIORITIZATION = Path("data/reporting/verified_pdf_candidate_residual_gap_prioritization.json")
DEFAULT_JSON = Path("data/reporting/bigster_equipment_page21_ambiguity_review.json")
DEFAULT_MARKDOWN = Path("data/reporting/bigster_equipment_page21_ambiguity_review.md")
PACKAGE_ID = "residual_gap_011"
...
NEXT_PACKAGE = "Jogger Equipment Page 21 Ambiguity Review"
DECISION_STATUSES = {"covered", "partially_covered"}
TRIMS = ("essential", "expression", "journey", "extreme")

class BigsterEquipmentPage21ReviewError(RuntimeError):
    pass
...
def ensure(condition: bool, message: str) -> None:
    if not condition:
        raise BigsterEquipmentPage21ReviewError(message)
...
def load_json_object(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise BigsterEquipmentPage21ReviewError(f"cannot read {label}: {exc}") from exc
    except json.JSONDecodeError as exc:
        raise BigsterEquipmentPage21ReviewError(f"invalid JSON in {label}: {exc}") from exc
...
def sha256(path: Path) -> str:
    ...
    try:
        ...
        raise BigsterEquipmentPage21ReviewError(f"cannot read archived source: {exc}") from exc
...
DECISIONS = ({'candidate_id': 'e3ea81e4d83c484bd7accca0086fcebf96ff8be8e801f060cfcf2a491a014356',
  'line_start': 39,
  'line_end': 39,
  'exact_text': 'Dach panoramiczny otwierany                                         -               -               '
                '¤             •',
  'decision': 'covered',
  'selected': [{'attribute_code': 'glass_sunroof', 'availability_status': 'not_available'},
               {'attribute_code': 'glass_sunroof', 'availability_status': 'optional'},
               {'attribute_code': 'glass_sunroof', 'availability_status': 'standard'}],
  'row_context': 'complete opening panoramic glass roof row',
  'source_availability': {'essential': 'not_available',
                          'expression': 'not_available',
                          'journey': 'optional',
                          'extreme': 'standard'},
  'rationale': 'The complete row prints the opening panoramic glass roof as unavailable in Essential and Expression, '
               'optional in Journey and standard in Extreme. All three attached signatures match the visual row and '
               'trim columns.'},)
...
def read_source_row(repository: Path) -> dict[str, str]:
    ...
    try:
        ...
        raise BigsterEquipmentPage21ReviewError(f"cannot read sources.csv: {exc}") from exc
...
def validate_prioritization(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    ensure(len(matches) == 1, "residual_gap_011 package differs")
    ...
    ensure(package.get("evidence_signature_count") == 3, "package evidence signature count differs")
    ensure(package.get("evidence_record_count") == 14, "package evidence record count differs")
...
def build_review(prioritization: Mapping[str, Any], repository: Path) -> dict[str, Any]:
    ...
    ensure((selected_signature_count, selected_record_count) == (3, 14), "selected evidence totals differ")
    return {
        ...
        "scope": {"candidate_count": 1, "source_code": SOURCE_CODE, "model_code": "bigster", "domain": "equipment_matrix", "page": SOURCE_PAGE, "input_coverage_status": "ambiguous"},
        ...
        "summary": {"candidate_count": 1, "decision_counts": {"covered": 1, "partially_covered": 0},
                    "selected_evidence_signature_count": selected_signature_count, "selected_evidence_record_count": selected_record_count,
                    "rejected_attached_signature_count": 3-selected_signature_count, "rejected_attached_record_count": 14-selected_record_count,
        ...
        "semantic_boundaries": {"review_is_not_import_approval": True, "bullet_option_and_dash_symbols_remain_distinct": True,
                                 "complete_row_trim_states_are_preserved": True, "panoramic_roof_optional_is_not_rewritten_as_standard": True,
                                 "no_configuration_projection_is_created": True},
        "next_package": {"name": NEXT_PACKAGE, "status": "planned",
                         "goal": "Review the 1 ambiguous equipment candidate from Jogger brochure page 21 against its 2 preserved evidence signatures without creating master-data rows or approved import specifications."},
    }

def render_markdown(payload: Mapping[str, Any]) -> str:
    summary = payload["summary"]
    lines = ["# Bigster Equipment Page 21 Ambiguity Review", "",
             "Authored review of `residual_gap_011`. The complete panoramic-roof row and all four trim states are preserved; the review does not approve imports.", "",
    ...
    lines.extend(["", "## Safety boundary", "", "- no file under `data/master` is changed;", "- no approved import specification is created or changed;",
                  ...
                  "- the Journey option marker is not promoted to standard;", "- unavailable, optional and standard panoramic-roof states remain trim-specific;",
...
def ensure_safe_output(repository: Path, path: Path) -> Path:
    ...
    for restricted in (repository / "data/master", repository / "data/imports"):
        try:
            ...
            continue
        raise BigsterEquipmentPage21ReviewError(f"output path is restricted: {path}")
    return resolved
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.verify:
            ...
            print("Bigster equipment page-21 ambiguity review: PASS")
        else:
        ...
        print("Selected evidence signatures: 3")
        print("Selected evidence records: 14")
        return 0
    except BigsterEquipmentPage21ReviewError as exc:
