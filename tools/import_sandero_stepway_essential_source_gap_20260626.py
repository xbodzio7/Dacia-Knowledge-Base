...
def verify_materialized() -> None:
    ...
    if summary["missing_technical_count"] != 137:
        raise ContractError(f"expected 137 remaining technical records, found {summary['missing_technical_count']}")
    if summary["exhausted_source_candidate_count"] != 2:
        raise ContractError("expected exactly two exhausted-source candidates")
    current = next(
