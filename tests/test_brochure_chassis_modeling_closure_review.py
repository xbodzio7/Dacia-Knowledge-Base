...
class BrochureChassisModelingClosureReviewTests(unittest.TestCase):
    ...
    def test_project_state_matches_completed_closure_package(self) -> None:
        state = json.loads((ROOT / "project" / "state.json").read_text(encoding="utf-8"))
        self.assertEqual(state["phase"], "Brochure Chassis Modeling Closure Review")
        self.assertEqual(state["current_package"]["status"], "complete")
        self.assertEqual(
            state["next_package"]["name"],
            "Official Brochure Technical Gap Resolution Closure Review",
        )
        self.assertEqual(state["baseline"]["tests"], 915)
