...
PATH = ROOT / "tools/bigster_equipment_page21_ambiguity_review.py"
SPEC = importlib.util.spec_from_file_location("bigster_equipment_page21_review", PATH)
...
class UnitTests(unittest.TestCase):
    ...
    def test_02_package_identity(self):
        ...
        self.assertEqual((package["package_id"], package["candidate_count"]), ("residual_gap_011", 1))

    def test_03_package_evidence_totals(self):
        package = review.validate_prioritization(self.prioritization)
        self.assertEqual((package["evidence_signature_count"], package["evidence_record_count"]), (3, 14))

    def test_04_wrong_kind(self):
        payload = copy.deepcopy(self.prioritization)
        payload["kind"] = "x"
        with self.assertRaisesRegex(review.BigsterEquipmentPage21ReviewError, "kind"):
            review.validate_prioritization(payload)

    def test_05_import_policy(self):
        payload = copy.deepcopy(self.prioritization)
        payload["policy"]["approved_import_spec_generation"] = True
        with self.assertRaisesRegex(review.BigsterEquipmentPage21ReviewError, "imports"):
            review.validate_prioritization(payload)

    def test_06_missing_signature(self):
        candidate = review.validate_prioritization(self.prioritization)["candidates"][0]
        with self.assertRaisesRegex(review.BigsterEquipmentPage21ReviewError, "not attached"):
            review.selected_signatures(candidate, [review.availability_signature("glass_sunroof", "package")])

    def test_07_manifest_length(self):
        self.assertEqual(len(review.DECISIONS), 1)

    def test_08_manifest_decision(self):
        self.assertEqual(review.DECISIONS[0]["decision"], "covered")

    def test_09_restricted_output(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "data/master").mkdir(parents=True)
            with self.assertRaisesRegex(review.BigsterEquipmentPage21ReviewError, "restricted"):
    ...
    def test_10_output_drift(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(review.BigsterEquipmentPage21ReviewError, "differs"):
...
class RepositoryTests(unittest.TestCase):
    ...
    def test_14_summary_decisions(self):
        self.assertEqual(self.payload["summary"]["decision_counts"], {"covered": 1, "partially_covered": 0})

    def test_15_summary_evidence(self):
        self.assertEqual((self.payload["summary"]["selected_evidence_signature_count"], self.payload["summary"]["selected_evidence_record_count"]), (3, 14))
    ...
    def test_17_attribute(self):
        self.assertEqual({item["signature"]["attribute_code"] for item in self.decision["selected_evidence_signatures"]}, {"glass_sunroof"})

    def test_18_statuses(self):
        self.assertEqual({item["signature"]["availability_status"] for item in self.decision["selected_evidence_signatures"]}, {"not_available", "optional", "standard"})

    def test_19_source_availability(self):
        self.assertEqual(self.decision["source_availability"], {"essential": "not_available", "expression": "not_available", "journey": "optional", "extreme": "standard"})
    ...
    def test_24_policy_and_next(self):
        ...
        self.assertTrue(self.payload["semantic_boundaries"]["panoramic_roof_optional_is_not_rewritten_as_standard"])
        self.assertEqual(self.payload["next_package"]["name"], "Jogger Equipment Page 21 Ambiguity Review")

    def test_25_markdown_boundary(self):
        self.assertIn("Journey option marker is not promoted to standard", self.markdown)
        self.assertIn("Jogger Equipment Page 21 Ambiguity Review", self.markdown)
