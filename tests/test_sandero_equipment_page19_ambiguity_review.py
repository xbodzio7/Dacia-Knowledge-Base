...
PATH = ROOT / "tools/sandero_equipment_page19_ambiguity_review.py"
SPEC = importlib.util.spec_from_file_location("sandero_equipment_page19_review", PATH)
...
class UnitTests(unittest.TestCase):
    ...
    def test_02_package_identity(self):
        ...
        self.assertEqual((package["package_id"], package["candidate_count"]), ("residual_gap_014", 1))
    ...
    def test_04_wrong_kind(self):
        ...
        payload["kind"] = "x"
        with self.assertRaisesRegex(review.SanderoEquipmentPage19ReviewError, "kind"):
            review.validate_prioritization(payload)

    def test_05_import_policy(self):
        payload = copy.deepcopy(self.prioritization)
        payload["policy"]["approved_import_spec_generation"] = True
        with self.assertRaisesRegex(review.SanderoEquipmentPage19ReviewError, "imports"):
            review.validate_prioritization(payload)

    def test_06_missing_signature(self):
        candidate = review.validate_prioritization(self.prioritization)["candidates"][0]
        with self.assertRaisesRegex(review.SanderoEquipmentPage19ReviewError, "not attached"):
            review.selected_signatures(candidate, [review.availability_signature("cruise_control", "package")])
    ...
    def test_09_restricted_output(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(review.SanderoEquipmentPage19ReviewError, "restricted"):
    ...
    def test_10_output_drift(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(review.SanderoEquipmentPage19ReviewError, "differs"):
...
class RepositoryTests(unittest.TestCase):
    ...
    def test_12_source_receipt_page(self):
        self.assertEqual(self.payload["source_receipt"]["page"], 19)
    ...
    def test_17_attribute(self):
        self.assertEqual({item["signature"]["attribute_code"] for item in self.decision["selected_evidence_signatures"]}, {"cruise_control", "speed_limiter"})

    def test_18_statuses(self):
        self.assertEqual({item["signature"]["availability_status"] for item in self.decision["selected_evidence_signatures"]}, {"standard"})

    def test_19_source_availability(self):
        self.assertEqual(self.decision["source_availability"], {"essential": "standard", "expression": "standard", "journey": "standard"})
    ...
    def test_24_policy_and_next(self):
        ...
        self.assertTrue(self.payload["semantic_boundaries"]["combined_row_maps_to_two_distinct_standard_attributes"])
        self.assertEqual(self.payload["next_package"]["name"], "Sandero Stepway Equipment Page 18 Ambiguity Review")

    def test_25_markdown_boundary(self):
        self.assertIn("`cruise_control` and `speed_limiter` remain two distinct attributes", self.markdown)
        self.assertIn("Sandero Stepway Equipment Page 18 Ambiguity Review", self.markdown)
