...
class DataProductReleaseContractTests(unittest.TestCase):
    ...
    def test_release_workflow_is_manual_write_and_read_only_on_prs(self) -> None:
        ...
        publisher_path = (
            REPOSITORY / "tools" / "publish_data_products_v1_15_0_20260804.sh"
        )
        recorder_path = (
            REPOSITORY
            / "tools"
            / "record_data_products_v1_15_0_publication_20260804.py"
        )
        if publisher_path.exists() or recorder_path.exists():
            ...
            self.assertIn(
                "data_products_v1_15_0_publication_001", publisher
            )
            self.assertIn("data-products-v1.15.0", publisher)
            ...
            self.assertIn("source_coverage_matrix_html", publisher)
            self.assertIn("Source coverage matrix", publisher)
            self.assertIn("gh release create data-products-v1.15.0", publisher)
            self.assertIn("gh release download data-products-v1.15.0", publisher)
            self.assertIn(
                "record_data_products_v1_15_0_publication_20260804.py",
                publisher,
            )
            self.assertIn(
                "release(data-products): record v1.15.0 publication",
                publisher,
            )
            self.assertIn("data_products_v1_15_0_publication", recorder)
            self.assertIn(
                "data_products_v1_15_0_publication.json", recorder
            )
            self.assertIn("public_v1_14_1_immutable", recorder)
            self.assertIn("source_coverage_matrix_html", recorder)
            self.assertIn(
                "post_v1_15_0_release_priority_selection_review_001",
                recorder,
            )
