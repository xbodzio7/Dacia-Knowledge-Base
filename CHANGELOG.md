...
* The automated test suite now contains 1747 tests.
* The verified master-data baseline now contains 46 CSV files and 11678 rows.
* SQLite verification now covers 46 tables and 11678 rows.
* Configuration attribute values now contain 3560 dated records.
* Declarative scalar configuration-value imports now contain 138 versioned JSON specifications.
* Configuration value ranges now contain 310 dated records from 24 range specifications.
* The canonical catalogue now contains 385 attributes in 30 categories.
* Equipment availability now contains 5903 records: 4589 `standard`, 514 `optional`, 800 `not_available` and 0 `unknown`.
